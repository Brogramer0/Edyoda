...
class Cinema:
    ...
    def buy_ticket(self, row, seats):
        divider = row / 2
        result = math.floor(divider)
        print("From row 1-",result, "price is $10" )
        print("From row ",result+1, "-", row,"price is $8" )
        ...
        if brow <= row and bcol <= seats:
            ...
            ans = input("You want to Book (y/n):")
            ch = ans.lower()
            if ch == "y":
                name = input("Enter Name:")
                gender = input("Enter Gender(M/F/Others):")
